-- Fix: rpc_register_collection_delivery still hitting RLS violations.
-- Root cause: SECURITY DEFINER runs as postgres, but auth.uid() may return
-- NULL in that context, causing auth-based RLS policies on collection_signatures,
-- collection_evidence, collections, and visit_tasks to deny writes.
-- Fix: SET role = 'service_role' guarantees BYPASSRLS on all tables.
