-- Replace get_facturacion_attribution() with get_facturacion_composicion()
-- Decomposes facturacion_actual (from ventas_odv) into 4 mutually exclusive categories
-- that always sum to 100%: M1, M2, M3, UNLINKED
-- Uses same causal criteria as get_impacto_botiquin_resumen():
--   M1 table totals: movimientos_inventario VENTA × medicamentos.precio (same as impacto)
--   M2: only ODVs AFTER first botiquín VENTA of that SKU
--   M3: only ODVs AFTER first CREACION, and doctor had NO prior ODV purchases of that SKU
-- Priority: M1 > M2 > M3 > UNLINKED (per odv row)
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH
  -- M1 chart: ODV IDs that came directly from botiquín sales via saga
  m1_odv_ids AS (
  ...
  -- M1 table: impacto-style (movimientos_inventario VENTA × medicamentos.precio)
  m1_impacto AS (
  ...
  -- First botiquín VENTA date per (cliente, sku) — for M2 temporal filter
  first_venta AS (
  ...
  -- First CREACION date for SKUs with CREACION but no VENTA — for M3
  first_creacion AS (
  ...
  -- M3 exclusion: (cliente, sku) pairs that had ODV purchases BEFORE or ON first_creacion
  prior_odv AS (
  ...
  -- Categorize each ventas_odv row with impacto-consistent criteria
  categorized AS (
    SELECT
      v.id_cliente,
      v.sku,
  ...
  -- Totals per client per category (for chart proportional allocation)
  totals AS (
    SELECT
      id_cliente,
      ...
      -- M2+M3 linked from ODV (for table, combined with m1_impacto)
  ...
  SELECT
    ...
    -- Proportional allocation for chart (decomposes facturacion_actual from ventas_odv)
    ...
    -- % growth: facturacion_actual vs baseline
    ...
    -- % linked to botiquín (M1 + M2 + M3) from ODV categorization
    CASE WHEN t.grand_total > 0
      THEN ROUND(((t.m1_total + t.m2_total + t.m3_total) / t.grand_total * 100)::numeric, 1)
      ELSE 0 END AS pct_vinculado,
    -- Table totals: M1 from impacto (movimientos × catálogo) + M2+M3 from ODV
