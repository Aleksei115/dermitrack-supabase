-- ============================================================
-- Fix: saga_status → workflow_status in rpc_compensate_visit_v2
--
-- Migration 000037 rewrote rpc_compensate_visit_v2 using the old
-- column name "saga_status", but migration 000014 already renamed
-- it to "workflow_status". This causes:
--   ERROR: column v.saga_status does not exist
--
-- 5 references fixed in rpc_compensate_visit_v2.
-- rpc_owner_delete_visit uses SELECT v.* so no fix needed.
-- ============================================================
