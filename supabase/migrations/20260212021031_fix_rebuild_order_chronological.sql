-- Fix rebuild ORDER BY: respetar orden cronológico real
--
-- Flujo real del corte: VENTA → LEV_POST_CORTE/RECOLECCION
-- El ORDER BY anterior agrupaba por día y priorizaba CREACION, lo cual era incorrecto.
-- Ahora se usa orden cronológico puro (created_at), y solo cuando el timestamp
-- es exactamente igual, se prioriza VENTA antes de CREACION/RECOLECCION.
...
BEGIN
  ...
  -- Con mismo timestamp exacto: VENTA primero, luego CREACION/RECOLECCION
  -- (refleja el flujo real: primero se vende, luego se hace levantamiento/recolección)
  FOR saga_rec IN
    ...
    ORDER BY created_at,
             CASE WHEN tipo IN ('VENTA','VENTA_ODV') THEN 0
                  WHEN tipo IN ('LEVANTAMIENTO_INICIAL','LEV_POST_CORTE','CORTE_RENOVACION') THEN 1
                  ELSE 2 END,
             id
  LOOP
