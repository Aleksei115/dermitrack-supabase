-- clasificacion_base(): Change > to >= for M2/M3 date comparisons
-- This includes same-day ODVs that occur on the same date as the first botiquín event.
-- Example: if a VENTA botiquín happens on 2026-01-30 and an ODV also on 2026-01-30,
-- the ODV now counts as a conversion (M2) instead of being excluded.
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  -- M2: M1 pairs that have ODV purchases ON OR AFTER first_venta (excluding saga)
  m2_agg AS (
  ...
  -- M3: Candidates with ODV ON OR AFTER first_creacion, no prior ODV purchases, excluding saga
  m3_agg AS (
