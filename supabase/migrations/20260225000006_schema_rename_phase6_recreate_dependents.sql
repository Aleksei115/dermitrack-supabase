...
DROP POLICY IF EXISTS inventory_select ON cabinet_inventory;
CREATE POLICY inventory_select ON cabinet_inventory FOR SELECT USING (can_access_client(client_id::text));
DROP POLICY IF EXISTS service_role_all ON cabinet_inventory;
...
DROP POLICY IF EXISTS "Admins can view status logs" ON client_status_log;
...
DROP POLICY IF EXISTS admin_collections_select ON collections;
...
DROP POLICY IF EXISTS admin_collection_evidence_select ON collection_evidence;
...
DROP POLICY IF EXISTS admin_collection_signatures_select ON collection_signatures;
...
DROP POLICY IF EXISTS "Users can insert saga_zoho_links" ON saga_zoho_links;
...
DROP POLICY IF EXISTS "Users can update saga_zoho_links" ON saga_zoho_links;
...
DROP POLICY IF EXISTS saga_compensations_select_authenticated ON saga_compensations;
CREATE POLICY saga_compensations_select_authenticated ON saga_compensations FOR SELECT USING (true);
DROP POLICY IF EXISTS saga_adjustments_select_authenticated ON saga_adjustments;
CREATE POLICY saga_adjustments_select_authenticated ON saga_adjustments FOR SELECT USING (true);
DROP POLICY IF EXISTS service_role_all ON event_outbox;
...
DROP POLICY IF EXISTS push_tokens_select ON user_push_tokens;
...
DROP POLICY IF EXISTS preferences_update ON user_notification_preferences;
...
DROP POLICY IF EXISTS config_read ON chatbot.config;
CREATE POLICY config_read ON chatbot.config FOR SELECT USING (true);
DROP POLICY IF EXISTS conversations_own ON chatbot.conversations;
